...
public class CodeGenerationUtils {
	 ...
	 * <p>
	 * Precondition: destination directory must terminate with a File.separator
	 * (e.g., '/' in unix-based systems)
	 * <p>
	...
	public static void buildTargetPackageDirectoryStructure(String destinationDirectory, String generatedPackage) {
		String destinationPath = generateGeneratedSourcePathFromPackageAndRootLocation(destinationDirectory,
				generatedPackage);
	 ...
	 * </ol>
	 * File.separator is used to delimit the path. All '.' in the package are
	 * replaced with the File.separator character.
	 * <p>
	 ...
	 * <p>
	 * Precondition: Package should not terminate with a File.separator (e.g.
	 * '/' in unix-based systems)
	 * 
	 * @param packageName
	 *            The java package for the generated java file.
	 * @param fileName
	 *            The name of the java file.
	 * @param fileContent
	 *            The content of the java file.
	 */
	public static void writeJavaClassFile(String destinationDirectory, String packageName, String fileName,
			String fileContent) {
		String filepath = generateGeneratedSourcePathFromPackageAndRootLocation(destinationDirectory, packageName)
		...
		try {
			if (file.exists() && file.isFile()) {
				...
				System.out.println("Path is invalid " + file.getCanonicalPath());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	/**
	 * Cleans name so that it conforms to Java identifier specification.
	 * 
	 * @param name
	 * @return
	 */
...
//		cleanedName = cleanedName.replaceAll("", "");
	 ...
	 * 
	 * @param classSource
	 *            - The class to which the field is to be added
	 * @param field
	...
	public static JavaInterfaceSource buildJavaInterface(ClassModel interfaceModel, String interfaceName) {
		final JavaInterfaceSource javaInterface = Roaster.create(JavaInterfaceSource.class);// Roaster.parse(JavaInterfaceSource.class,
																							// "import
																							// java.util.Map;
																							// public
																							// interface
		...
		for (Method accessor : interfaceModel.getClassMethods()) {
			...
			for (String importString : accessor.getImports()) { // TODO remove
																// when Roaster
																// fixes bug
				javaInterface.addImport(importString);
			}
			MethodSource<JavaInterfaceSource> methodSource = javaInterface.addMethod().setPublic()
	...
	public static JavaClassSource buildJavaClass(ClassModel classModel, String adapterName) {
		...
		if (classModel.getSupertypes() != null && classModel.getSupertypes().size() > 0
				&& StringUtils.isNotBlank(classModel.getSupertypes().get(0))) { // Java
																				// supports
																				// only
																				// single
																				// inheritance
	...
	/**
	 * Returns the last component of a '.' delimited path. For instance, for the
	 * following path:
	 * 
	 * <code>
	 * java.lang.String
	...
	/**
	 * Method returns the suffix of a path starting from the resource of
	 * interest. For instance, for the path 'Condition.id' and the prefix
	 * 'Condition', this method returns the string 'id' as the suffix. It is the
	 * path relative to the containing resource.
	 * 
	 * @param prefix
	 *            - The prefix, generally the name of the resource
	 * @param attributePath
	 *            - The full attribute path as defined in the profile
	 * @return
	 */
	...
	/**
	 * Method takes a '.' delimited invocation path and returns a list of
	 * component split by the '.' delimiter.
	 * 
	 * For instance, Patient.contact.telecom.extension will be split and
	 * returned as list {Patient, contact, telecom, extension}
	 * 
	 * @param path
	 * @return
	 */
