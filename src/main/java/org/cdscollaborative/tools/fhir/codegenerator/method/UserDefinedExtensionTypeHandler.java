...
public class UserDefinedExtensionTypeHandler extends BaseExtensionMethodHandler {
	...
	public UserDefinedExtensionTypeHandler(FhirResourceManager manager, CodeTemplateUtils template, StructureDefinition profile, ElementDefinitionDt element) {
...
//		accessors.add(constructSetMethodFromField(attributeName, getFullyQualifiedType()).setBody(buildExtendedTypeSetterBody(getExtensionUri())));
	...
	protected void handleMultipleCardinality(List<Method> accessors) {
		...
		accessors.add(constructSetMethodForMultiCardinalityField(getExtendedElement().getName(),getFullyQualifiedType()).setBody(buildUserDefinedExtensionTypeSetterBody(getFullyQualifiedType())));
	}
